...
public class Chessman : MonoBehaviour
{
    ...
    // da preveri, če se kralj se ni premaknil
    ...
    private void CastlingMovePlates()
{
    ...
    // samo kralj
    if (!name.EndsWith("_king")) return;

    // kralj se ne sme bit že premaknil
    if (hasMoved) return;

    // kralj mora biti na začetni poziciji (po tvojem setupu)
    int homeY = (player == "white") ? 0 : 7;
    if (yBoard != homeY || xBoard != 4) return;

    // kingside: e->g (x 4 -> 6), rook h->f (7 -> 5)
    TryAddCastle(g, kingSide: true);

    // queenside: e->c (4 -> 2), rook a->d (0 -> 3)
    TryAddCastle(g, kingSide: false);
}

private void TryAddCastle(Game g, bool kingSide)
{
    int y = yBoard;
    ...
    // rook mora obstajati, biti rook in isti player
    GameObject rookObj = g.GetPosition(rookX, y);
    if (rookObj == null) return;

    var rookCm = rookObj.GetComponent<Chessman>();
    if (rookCm == null) return;
    if (!rookCm.name.EndsWith("_rook")) return;
    if (rookCm.player != player) return;
    if (rookCm.hasMoved) return;

    // polja med kraljem in rookom morajo biti prazna
    if (kingSide)
    {
        // f1/g1 ali f8/g8
        if (g.GetPosition(5, y) != null) return;
        if (g.GetPosition(6, y) != null) return;
    }
    else
    {
        // d1/c1/b1 ali d8/c8/b8
        if (g.GetPosition(3, y) != null) return;
        if (g.GetPosition(2, y) != null) return;
        if (g.GetPosition(1, y) != null) return;
    }

    // OK -> pokaži potezo kralja za 2 polji
    MovePlateSpawn(kingTargetX, y);
}
